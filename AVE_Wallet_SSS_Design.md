...
| Share A 坐标规则 | 初始创建时 x=1；换机恢复时分配新坐标：从 [4, 99] 范围内取随机整数（避开 1/2/3），钱包服务记录已用坐标防碰撞；也可改用自增序列（4, 5, 6…） |
...
```
坐标约定：Share A x=1（初始）/ x=r（换机，r∈[4,99]随机）；Share B x=2；Share C x=3

给定 B = f(2), C = f(3)：
  a₁ = C - B
  S  = 3B - 2C              ← BIP39 熵（16/32 字节）
  A  = f(1) = 2B - C        ← 初始设备 Share A（x=1）
  A' = f(r) = S + a₁ * r   ← 新设备 Share A（x=r，r 由钱包服务分配，r∈[4,99]）
...
> **新设备坐标 r 的分配规则：**
> - 固定坐标 1/2/3 保留给初始 Share A / B / C，不可复用
> - 换机时钱包服务为该钱包从 [4, 99] 中分配一个未使用的随机整数 r，并将 r 记录在钱包表中防止碰撞
> - 也可改为自增序列（4 → 5 → 6…），逻辑更简单，推荐后端使用自增序列实现
...
| 分片坐标约定 | Share A x=1（初始）/ x=r（换机，r∈[4,99]）；Share B x=2（固定）；Share C x=3（固定）；三端定义相同 |
...
步骤3：用户设备本地重建助记词
  ├─ 解密得到 Share A 明文
  ├─ Share A + Share B（从平台云或本地备份获取）→ Lagrange → BIP39 熵（16/32 字节）
...
AI Agent 发起交易请求
    → 风控校验（§10.5）
    → 平台云取 Share B + 钱包服务 KMS 取 Share C
    → Lagrange 重建 Master Seed → BIP44 派生私钥 → 签名
    → Share B / C 明文立即销毁
    → 广播上链
```
...
EOA 钱包没有平台云助记词备份功能，Share B 仅为 Agent 签名期间临时存储，因此**关闭时必须删除平台云 Share B**，无例外。
...
用户触发"关闭 Agent 交易"
    ...
    ├─ 1. Passkey 验证
    │
    ├─ 2. 删除平台云 Share B（必须执行，无条件）
    │       └─ EOA 无平台云备份场景，Share B 始终删除
    │
...
| 模块 | 内容 | 优先级 |
|------|------|-------|
| 设备端 SDK | BIP39 生成、SSS 分片、TEE 写入、ECDH 加密上传 | P0 |
| 钱包服务 | Share B 转存平台云、Share C 写 KMS、Agent 签名流程（§10）| P0 |
| 平台云服务 | Share B 加密存储、按 credit_id 授权查询 | P0 |
| 风控模块 | 进程内 library，分级限频、签名校验（§10.5）| P0 |
| Session Credit | 授权许可生成、撤销、有效期管理（§十一）| P0 |
| 钱包恢复流程 | Share B/C 重建 + 新设备 TEE 写入（§七）| P1 |
| 本地签名 | TEE 预派生私钥直签（§九）| P1 |
| 删除钱包 | 软删除 + 60 天物理删除（§7.5）| P1 |

**产品建议：**
- SSS 钱包作为**默认新建钱包类型**，在引导页突出"无需备份助记词"的安全卖点
- Bot 钱包和 EOA 钱包保持现有入口，不做下线，确保存量用户不受影响
- Agent 自动交易功能仅对 SSS 钱包开放
...
| Bot 钱包导出 | B+C 重建 → 生成 Share A → 设备端重建助记词导出（§12.7）| P1 |
