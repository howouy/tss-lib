...
> 基于 SSS_Original_Design.md，结合 TSS 调研结论，细化 SSS 实现路径。
> 核心判断：当前阶段采用 **SSS + 设备 TEE** 方案，不使用 TSS/MPC。
...
| 库 | 语言 | 特点 | 适用 |
|----|------|------|------|
| `hashicorp/vault/shamir` | Go | 生产级，Vault 在用，GF(256) | 后端 Server TEE（推荐） |
| `corvus-ch/sss` | Go | 轻量，标准 Shamir | 后端 |
| `tss-lib/crypto/vss` | Go | 已验证，含 Lagrange | 本项目已有，可复用 |
| `secrets.js` | JS/TS | 前端可用 | Web 端 |
| `slip39` | 多语言 | Shamir Backup 标准（Trezor） | 若需对标 SLIP-39 |

**推荐：** 后端用 `hashicorp/vault/shamir`；移动端通过 gomobile 封装 Go SSS 逻辑复用。
---
...
| 有导出需求的用户 | 走私钥导出流程（§12.6 待写），导出后作为本地 EOA 钱包使用 | 观察期内提供导出功能入口 |
