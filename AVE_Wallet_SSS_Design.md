...
### 14.1 架构决策：平台云不需要 Nitro Enclave
...
### 14.4 Wallet Service Enclave 请求 Share B（Agent 签名）
...
| App 上传接口 | AVE Token 验证、Master Key 签名校验、ECDH 解密、KMS 加密存储 Share B（§14.2）| P0 |
| App 下载接口 | AVE Token + 2FA 验证、KMS 解密 Share B、ECDH 加密回传、限频（§14.3）| P0 |
| Wallet Enclave 接口 | NSM Attestation PCR 验证、credit_id 有效性校验、KMS 解密 Share B、TLS 回传（§14.4）| P0 |
| AWS PrivateLink 配置 | Endpoint Service（NLB）创建、钱包服务账号跨账号白名单授权（§10.8）| P0 |
| Share B 生命周期管理 | purpose 区分（backup / agent）、软删除、30 天物理删除、到期自动清理（§14.5）| P0 |
...
### 15.4 第三阶段：EOA 助记词钱包升级支持 Agent 交易
...
EOA 升级上传 Share B 与 Keyless 钱包 Agent 激活走同一接口（§14.2），关闭 Agent 时 Share B / Share C 软删除走同一生命周期管理逻辑（§14.5），无需额外开发。
...
| 8 | **EOA 助记词钱包开通 Agent 是否可选** | §13.1、§15.4 | 当前设计为可选功能，不强制，用户主动选择是否上传分片。是否需要产品引导路径设计（如首次使用 Agent 时弹出引导）？ |
