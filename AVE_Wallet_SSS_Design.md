...
```
钱包服务调平台云服务取 Share B（x=2）
钱包服务从 KMS 读取 Share C（x=3）
    ↓
...
步骤2：服务端计算 Share A 并下发
  ├─ 从平台云取 Share B（x=2），从 KMS 读取 Share C（x=3）
  ├─ Lagrange 插值恢复 BIP39 熵 S 及系数 a₁
  ├─ 生成新坐标 x=n（从 x=4 自增，记录至钱包表防碰撞）
  ├─ 计算 Share A = f(n) = S + a₁·n（GF-256）
  ├─ 用用户设备临时公钥 ECDH 加密后下发
  └─ S、a₁ 及所有中间值立即销毁

步骤3：设备本地存储 Share A
  ├─ 解密得到 Share A 明文
  ├─ Share A 写入设备 TEE（与 Keyless 钱包创建流程一致）
  └─ 内存中 Share A 明文立即清除
...
步骤2：服务端生成 Share A（ECDH 加密下发）
  ├─ 钱包服务调平台云取 Share B（x=2），从 KMS 读取 Share C（x=3）
  ├─ B + C → Lagrange → BIP39 熵 S 及多项式系数 a₁
  ├─ 生成新坐标 x=n（从 x=4 开始自增，与 Keyless §7.4 规则一致，记录至钱包表防碰撞）
  ├─ 计算 Share A = f(n) = S + a₁·n（GF-256 运算）
  ├─ 用用户设备临时公钥 ECDH 加密 Share A 后下发
  └─ S、a₁ 及所有中间值立即销毁，Share B / C 密文留存至导出确认
...
| 钱包恢复流程 | Share B/C 重建 + 新设备 TEE 写入（§七）| P0 |
...
| 3 | **存量 Bot 钱包最终命运** | §12.5 | 路线 A：长期共存，不强制关闭，用户自愿升级；路线 B：设 3 个月缓冲期后关闭签名服务，强制升级。取决于存量规模和运营策略。 |
| 4 | **Bot 钱包缓冲期结束后未升级用户如何处理** | §12.5 | 路线 B 下，缓冲期满仍未升级的用户：签名服务关闭后资产锁定？是否提供额外宽限？数据保留多久？ |
