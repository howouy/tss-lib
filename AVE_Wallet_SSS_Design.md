...
步骤3：客户端发起恢复请求（单次请求）
  ...
  ├─ 用钱包服务公钥 ECDH 协商会话密钥，加密 Share B
  └─ 上传至钱包服务：{ enc_B, ePub_client, 账号 Token, 2FA 凭证 }
     （ePub_client 用于钱包服务加密回传 A'，一次请求完成所有交互）
...
步骤4：钱包服务内部处理（Share C 全程不出钱包服务）
  ...
  ├─ 分配新设备坐标 new_x（自增，从 4 开始），计算 A' = S + a₁ * new_x
  ├─ 用 ePub_client 加密 A' 后回传
  ├─ B 明文、S、a₁ 立即销毁，C 不变，new_x 记录至钱包表
  └─ 返回 { enc_A', new_x }
...
| `created_at / expire_at` | Unix 时间戳；到期后后端软删除 Share B（30 天后物理删除） |
...
步骤4：用户确认导出，通知服务端清理
  ...
  ├─ 软删除平台云 Share B（30 天缓冲期后物理删除，与 §8.1 一致）
